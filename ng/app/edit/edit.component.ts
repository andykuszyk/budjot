...
export class EditComponent extends AuthBase {
    ...
    constructor(http: HttpClient, authService: SocialAuthService, private route: ActivatedRoute) {
        super(http, authService);
        this.budjot = new Budjot('', 0);
        this.newEntry = new BudjotEntry('', 0, false, () => this.budjot.updateFields());
    }
    ...
    protected override onInit() {
        ...
        if(this.id == 'new') {
            ...
            this.http.get<any>(url, { headers: new HttpHeaders({'Content-Type': 'application/json', 'Authorization': this.idToken}), observe: 'response' }).subscribe(res => {
                console.log(res);
                this.budjot = new Budjot(res.body['name'] as string, res.body['income']);
    ...
    save() {
        ...
        this.http.post<any>(url, this.budjot.toJson(), { headers: new HttpHeaders({'Content-Type': 'application/json','Authorization': this.idToken}), observe: 'response'}).subscribe(
            res => { 
                this.id = res['body']!['id'];

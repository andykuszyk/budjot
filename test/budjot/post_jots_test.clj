...
(deftest post-jots-returns-201-and-body
  (let [response (client/post f/budjot-url (f/build-post-jot-request (f/build-jot "new jot")))]
...
(deftest post-jots-with-same-name-returns-409
  (is (= 201 (:status (client/post f/budjot-url (f/build-post-jot-request (f/build-jot "409"))))))
  (is (thrown-with-msg? Exception #"status 409" (client/post f/budjot-url (f/build-post-jot-request (f/build-jot "409"))))))

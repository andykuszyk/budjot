...
(deftest get-jot-by-id-returns-200-and-jot
  (let [
        posted-jot (post-jot)
        got-jot (json/read-str (:body (client/get (format "%1s/%2s" (budjot-url) (:_id posted-jot)))) :key-fn keyword)
        ]
...
(deftest get-jots-returns-200-and-jots
  (let [
        ...
        got-jots (json/read-str (:body (client/get (budjot-url))) :key-fn keyword)
        ]
